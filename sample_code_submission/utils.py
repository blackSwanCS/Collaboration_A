...
def plot_calibration_curve(
    ...
    data_denH, weight_denH, data_numH, weight_numH,
    path_to_figures="", nbins=100, epsilon=1.0e-20,
    label="Calibration Curve", score_range="standard", save=""
):
    ...
    h_sumH = h_SH + h_XH
    h_ratioH = np.divide(h_XH, h_sumH + epsilon)
    errH = np.sqrt((h_XH_err / (h_XH + epsilon)) ** 2 + (h_SH_err / (h_SH + epsilon)) ** 2)
    ...
    brier_holdout = np.average((preds_holdout - labels_holdout) ** 2, weights=weights_holdout)

    # --- Plot ---
    fig = plt.figure(figsize=(12, 8), constrained_layout=True)
    ax1 = fig.add_subplot(2, 1, 1)
    hep.histplot([h_ratio, h_ratioH], bins=edges, yerr=[err, errH],
                 label=["Training", "Holdout"])
    abline(1.0, 0.0)
    plt.title(label, fontsize=12)

    # Text metrics
    plt.text(x=0.2, y=1.0, s=fr"${{\chi}}^2_{{\rm Train}}/n_{{\rm dof}}$ = {chi_2:.3f}")
    plt.text(x=0.2, y=0.92, s=fr"${{\chi}}^2_{{\rm Holdout}}/n_{{\rm dof}}$ = {chi_H:.3f}")
    plt.text(x=0.2, y=0.84, s=fr"Brier (Train) = {brier_train:.4f}")
    plt.text(x=0.2, y=0.76, s=fr"Brier (Holdout) = {brier_holdout:.4f}")
...
def plot_three_score_distributions(nominal_scores, plus_scores, minus_scores,
                                  bins=50, range=(0,1), save_path=None):
    """
    ...
    plt.hist(nominal_scores, bins=bins, range=range, histtype='step', linewidth=2,
             label='Nominal', color='black', density=True)

    plt.hist(plus_scores, bins=bins, range=range, histtype='step', linewidth=2,
             label='Plus variation', color='red', density=True)

    plt.hist(minus_scores, bins=bins, range=range, histtype='step', linewidth=2,
             label='Minus variation', color='blue', density=True)
...
def plot_three_systematics_calibration(nominal_scores, nominal_labels, nominal_weights,
                                      plus_scores, plus_labels, plus_weights,
                                      minus_scores, minus_labels, minus_weights,
    ...
    x_nom, y_nom = compute_fraction_shifted(nominal_scores, nominal_labels, nominal_weights, bins)
    x_plus, y_plus = compute_fraction_shifted(plus_scores, plus_labels, plus_weights, bins)
    x_minus, y_minus = compute_fraction_shifted(minus_scores, minus_labels, minus_weights, bins)
